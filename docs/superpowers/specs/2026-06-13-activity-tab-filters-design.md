...
- Lives on the SwiftUI view's `@State`, so it persists for the lifetime of the view (i.e. across tab switches while the app is alive) but resets to `.all` on a fresh app launch. No `UserDefaults` persistence — if this turns out to be annoying in practice, we can add it as a one-line follow-up.
...
- `podcasts/Strings+Generated.swift`
  - Regenerated by the L10n build phase on the next `make build`. If the script doesn't pick up the new key automatically, manually add the `L10n.activity` accessor following the existing pattern in that file.
